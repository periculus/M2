...
-- Put it all together
load "GTVlambdaotimesVlambdastar.4.m2";
...
Vlambda = irreducibleLieAlgebraModule(lambda,g);
LAB = lieAlgebraBasis(g);
time Llambda = GTrepresentationMatrices(Vlambda);
rhoVlambda = lieAlgebraRepresentation(Vlambda,LAB,Llambda);
lambdastar = starInvolution(lambda,g);
Vlambdastar = irreducibleLieAlgebraModule(lambdastar,g);
time Llambdastar = GTrepresentationMatrices(Vlambdastar);
rhoVlambdastar = lieAlgebraRepresentation(Vlambdastar,LAB,Llambdastar);
time hwv = first weightNuHighestWeightVectorsInVtensorW({0,0,0},rhoVlambda,rhoVlambdastar);
R = ring hwv;
Blambda = gtPatterns("A",dynkinToPartition("A",lambda));
Blambdastar = gtPatterns("A",dynkinToPartition("A",lambdastar));
...
for i from 0 to N-1 do (
    ...
    b = molevCoefficient(gtp(p1));
    coeff=lift(1/(sqrt(a*b)),QQ);
    sgn = (-1)^(1+level((gtp(p0))#"weight",g#"RootSystemType",g#"LieAlgebraRank"));
    f = f + sgn*coeff*R_i*R_(N+j)
);
scale(f)==scale(hwv)
-- true

18*A_0*B_9-18*A_1*B_8+18*A_2*B_7+12*A_3*B_6-12*A_4*B_5+6*A_5*B_4-6*A_6*B_3+6*A_7*B_2-3*A_8*B_1+A_9*B_0
...
time Llambdastar = GTrepresentationMatrices(Vlambdastar);
rhoVlambdastar = lieAlgebraRepresentation(Vlambdastar,LAB,Llambdastar);
time hwv = first weightNuHighestWeightVectorsInVtensorW({0,0,0,0,0},rhoVlambda,rhoVlambdastar);
R = ring hwv;
Blambda = gtPatterns("A",dynkinToPartition("A",lambda));
Blambdastar = gtPatterns("A",dynkinToPartition("A",lambdastar));
...
time for i from 0 to N-1 do (
 ...
 -- used 124.804s (cpu); 53.1009s (thread); 0s (gc)
f = scale(f);

fn = openOut "V00000inV11011otimesV11011.m2";
fn << toString(f) << endl;
close fn;
