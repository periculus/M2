...
// 11. Multiline TripleString spanning divider lines
//     Simulates PrimaryDecomposition/examples.m2 pattern:
//     58-slash divider + text + 58-slash divider
{
  const divider = '/'.repeat(58);
  const code = divider + '\nsome text between\n' + divider;
  const ts = findNode(code, 'TripleString');
  assert(ts !== null, `58-slash divider pair should produce TripleString`);
  assert(errorCount(code) === 0, `58-slash divider pair should have 0 errors, got: ${errorCount(code)}`);
  // TripleString should span the entire input (not self-close on first line)
  assert(ts.from === 0 && ts.to === code.length,
    `58-slash pair TripleString should span entire input (from=${ts?.from}, to=${ts?.to}, len=${code.length})`);
}

// 12. Ensure M2's //// escape still works with greedy body
{
  const code = '///before //// after///';
  const ts = findNode(code, 'TripleString');
  assert(ts !== null, `"${code}" should produce TripleString`);
